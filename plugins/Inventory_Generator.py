...
class InventoryInstance(StrippedAwsInstance):
    def __init__():
        super().__init__()

        if self.source_instance.is_valid:
            self.id = self.source_instance.get_id()
            self.name = self.source_instance.get_name()
            self.is_valid = True

class InventoryInstanceGroup(InstanceGroup):
    def __init__(self, ec2_client=None, initial_instances: Optional[List[str]] = None):
        ...
        self._instances: Set[InventoryInstance] = set()

    def get_instance_objects(self) -> Set[InventoryInstance]:
...
class AWSInventoryPlugin(BasePlugin, cmd2.CommandSet):
    """AWS plugin for generating inventory."""
    ...
    def __init__(self, initial_instances: Optional[List[str]] = None):
        ...
        self._instance_group = InventoryInstanceGroup(ec2_client=self.ec2_client, initial_instances=initial_instances)
        self._command_handler = InventoryCommandHandler(ssm_client=self.ssm_client)
